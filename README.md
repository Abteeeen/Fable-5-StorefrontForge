...
(vanilla JS), ad creatives rendered as HTML→PNG (Playwright), cheap-model copy drafting and
ad scoring (OpenRouter).
...
node forge/score.mjs                   # rank creatives (OpenRouter or fallback)
```
...
| `forge/score.mjs` | Creative scoring via OpenRouter vision model |
| `forge/copywriter.mjs` | Brief → brand config draft via OpenRouter |
